...
Platforms (Windows, Mac, Linux) have different ways of defining variables at
the system-level, per account or for a terminal session.
...
> [!warning]
> Do not use system/account-wide environment variables for high-value secrets
> (production/admin/etc tokens/keys/passwords). Session-level variables may
> also be insecure.
...
[pass](https://www.passwordstore.org/), etc.

Example using Bitwarden CLI (note is [dotenv](https://www.dotenv.org/docs/security/env.html) formatted):
```bash
