...
with its surrounding environment. Interaction can include gathering information
...
tools may refuse to do both simultaneously.
...
With that knowledge, we can break down the workflow into phases:
...
To get the current time for step 1, we use a language model to extract the zone
...
use *Tools &rsaquo; Invoke Tool*. This allows the workflow to modify or
transform the tool arguments before the tool implementation is executed.
...
To finish, we need to reconnect the Start and Finish nodes so that the workflow
acts on the user prompt and displays messages in the Chat tab. However, there
are multiple ways to organize messages in the final history.

If the *Chat* node extends the conversation, then all intermediate messages
will appear as a flattened sequence in the history. We might not necessarily
want to see messages in between the original question and the final answer.
...
Once you are satisfied with the workflow, remember to reconnect the `input` pin
of the *Start* node to the *Chat* node.
...
composing smaller workflows using subgraphs and chaining.

In the following set of tutorials we will explore using subgraphs to organize
more complex workflows before covering chained workflows.
