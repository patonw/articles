...
Inside the subgraphs, the editor contain a set of nodes and wires, including a Start and Finish node.
...
Similarly to the Simple Subgraph, change replace the text output pin with a JSON value.
...
> [!tip]
>You could also reorder pins to minimize crossing wires, but don't do that yet.
...
You may have noticed the `parallel` option on the Iterative Subgraph.

Enabling this will launch a pool of subgraph runners that will split the list
items among themselves and execute simultaneously. In many cases, this can
result in a significant speed up. However, there are risks.

> [!CAUTION]
> Batches of 5 items as used in this example shouldn't be a problem. With
> larger lists and remote APIs, however, you will encounter API rate limits.
>
> This will likely result in a failure rather than slowing down the workflow.
